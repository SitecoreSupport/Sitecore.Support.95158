...
Sitecore.LayoutDefinition.moveToPosition = function (uid, placeholderKey, position) {
    ...
    var placeholderWiseCountEmpty = 0;
    for (var totalCount = 0; totalCount < device.r.length; totalCount++) {
        ...
        if (renderingPlaceholder == '' && placeholderWiseCount > 0) {
            placeholderWiseCountEmpty++;
        }
        ...
        if (placeholderWiseCountEmpty + placeholderWiseCount == position) {
            r["@ph"] = placeholderKey; // patch 414405
            Sitecore.LayoutDefinition.handleRelatedRenderings(placeholderKey, rendering, device.r);
...
Sitecore.LayoutDefinition.handleRelatedRenderings = function (newPlaceholder, renderingToMove, renderings) {
    ...
    for (var i = 0; i < renderings.length; i++) {
        ...
        if (renderingPlaceholder.match("^" + movedRenderingPlaceholder)) {
            r["@ph"] = r["@ph"].replace(renderingToMove["@ph"], newPlaceholder)
        }
    }
};
